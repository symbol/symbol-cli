...
describe('OptionsResolver', () => {
    ...
    it('should return the value', async () => {
        const value = await OptionsResolver({ name: 'nem' }, 'name', () => undefined, 'Insert your name', 'text', undefined);
    ...
    it('should return the value (int)', async () => {
        const value = await OptionsResolver({ name: 0 }, 'name', () => undefined, 'Insert your name', 'text', undefined);
        expect(value).to.be.equal(0);
    });

    it('should return the secondSource value', async () => {
        const value = await OptionsResolver({}, 'name', () => 'nem', 'Insert your name', 'text', undefined);
        expect(value).to.be.equal('nem');
    });
    ...
    it('should prompt the user when provided a wrong option key', async () => {
        ...
        const value = await optionsResolver.OptionsResolver({ dummy: 'dummy' }, 'name', () => undefined, '', 'text', undefined);
...
describe('OptionsChoiceResolver', () => {
    ...
    it('should return the value', async () => {
        ...
        const value = await OptionsChoiceResolver({ name: 'nem' }, 'name', 'Select name:', choices, 'select', undefined);
        expect(value).to.be.equal(0);
    });

    it('should pass validation', async () => {
        const choices = [{ title: 'Remove', value: ActionType.Remove }];
        const value = await OptionsChoiceResolver({ name: 'Remove' }, 'name', 'Select name:', choices, 'select', new ActionValidator());
    ...
    it('should prompt the user when provided a wrong option key', async () => {
        const stub = sinon.stub().returns({ name: 'user was prompted' });
        const optionsResolver = proxyquire('../src/options-resolver', { prompts: stub });
        const value = await optionsResolver.OptionsChoiceResolver({ dummy: 'dummy' }, 'name', 'test', [], 'select', undefined);
        expect(value).equal('user was prompted');
    });
...
describe('OptionsConfirmationResolver', () => {
    it('should return the value', async () => {
        const value = await OptionsConfirmResolver({ name: true }, 'name', 'test', 'confirm', true, 'value');
    ...
    it('should prompt the user when provided a wrong option key', async () => {
        ...
        const optionsResolver = proxyquire('../src/options-resolver', { prompts: stub });
        const value = await optionsResolver.OptionsConfirmResolver({ dummy: 'dummy' }, 'name', 'test', 'confirm', true);
        expect(value).equal('user was prompted');
    });
});
