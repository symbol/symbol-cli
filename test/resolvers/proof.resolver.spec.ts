...
describe('Proof resolver', () => {
    it('should return proof', async () => {
        ...
        expect(await new ProofResolver().resolve({ proof } as any, LockHashAlgorithm.Op_Hash_160)).to.be.equal(proof);
    });
    ...
    it('should change key', async () => {
        ...
        expect(await new ProofResolver().resolve(options, undefined, 'altText', 'key')).to.be.equal(key);
    });
});
