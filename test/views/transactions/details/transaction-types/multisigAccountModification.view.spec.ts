...
describe('MultisigAccountModificationView', () => {
    it('should return a view', () => {
        ...
        expect(view['Address addition (1 / 2)']).equal(account1.address.pretty());
        expect(view['Address addition (2 / 2)']).equal(account2.address.pretty());
        expect(view['Address deletion (1 / 1)']).equal(account3.address.pretty());
    });
});
