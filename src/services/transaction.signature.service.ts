 ...
 */
import {
   Account, Transaction, SignedTransaction, UInt64,
   AggregateTransaction, Deadline, LockFundsTransaction,
   NetworkType, PublicAccount, MultisigAccountInfo,
} from 'symbol-sdk'
...
export class TransactionSignatureService {
   ...
   get generationHash(): string { return this.profile.networkGenerationHash }
   get networkType(): NetworkType { return this.profile.address.networkType }
   get networkCurrency(): NetworkCurrency { return this.profile.networkCurrency }
   get url(): string { return this.profile.url }
   ...
   get announceMode(): AnnounceMode {
      ...
      if (this.isAggregateBonded) { return AnnounceMode.partial }
      if (!this.signerMultisigInfo) { return AnnounceMode.standard }
      if (this.signerMultisigInfo.minApproval > 1) { return AnnounceMode.partial }
      return AnnounceMode.complete
   }
   ...
   private constructor(
      ...
      private readonly options: AnnounceTransactionsOptions,
   ) {}
   ...
   public async signTransactions(args: TransactionSignatureOptions): Promise<SignedTransaction[]> {
      ...
      if (signerMultisigInfo) { this.signerPublicAccount = signerMultisigInfo.account }
   ...
   private async signPartialTransactions(
      account: Account,
   ...
   private async createHashLockTransaction(
      aggregateTx: SignedTransaction,
