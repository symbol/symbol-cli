...
import {
   Transaction, SignedTransaction, Listener,
   TransactionHttp, TransactionAnnounceResponse, TransactionType,
} from 'symbol-sdk'
...
export class TransactionAnnounceService {
    ...
    * @type {string}
    */
   public get url(): string { return this.profile.url }
   ...
   public async announce(transactions: SignedTransaction[]): Promise<void> {
      ...
      if (!await new AnnounceResolver().resolve(this.options)) { return }
      ...
      if (normalTx.length) { this.announceTransactions(normalTx) }
      if (hashLockTx.length) { this.announceAggregateTransaction(hashLockTx) }
   }
   ...
   private announceTransactions(transactions: SignedTransaction[]) {
      if (this.options.sync) {
         for (const tx of transactions) { this.announceTransactionSync(tx) }
         return
      }

      for (const tx of transactions) { this.announceTransaction(tx) }
   }
      ...
      listener.open().then(() => {
         merge(
            ...
            listener
               ...
               .pipe(
                  filter((transaction) => transaction.transactionInfo !== undefined
                     && transaction.transactionInfo.hash === hashLock.hash),
