...
export class SequentialFetcher {
     ...
     */
    static create(networkCallFunction: (address: Address) => Promise<any>, minDelay = SEQUENTIAL_FETCHER_DEFAULT_DELAY): SequentialFetcher {
     ...
     */
    private constructor(private networkCallFunction: (address: Address) => Promise<any[]>, private minDelay: number) {
    ...
    private startFetchingRoutine() {
        this.routineController.pipe(startWith(true)).subscribe(async (continueRoutine) => {
            if (!continueRoutine) {
