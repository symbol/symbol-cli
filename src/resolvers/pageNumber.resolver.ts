 ...
 */
export class PageNumberResolver implements Resolver {
    /**
     * Resolves an page number provided by the user.
    ...
    async resolve(options: Options, altText?: string, altKey?: string): Promise<number> {
        const resolution = +(await OptionsResolver(
            options,
            altKey ? altKey : 'pageNumber',
            () => undefined,
            altText ? altText : 'Enter page number:',
            'number',
